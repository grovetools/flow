...
// TestPopupRowsWhileArmed asserts the View namespace surfaces its 13 completions
...
func TestPopupRowsWhileArmed(t *testing.T) {
	...
	if len(rows) != 13 {
		t.Fatalf("PendingRows(\"v\") = %d rows, want 13", len(rows))
	}
	wantSuffix := map[string]bool{"l": true, "f": true, "b": true, "v": true, "t": true, "c": true, "m": true, "a": true, "s": true, "y": true, "j": true, "o": true, "e": true}
	for _, r := range rows {
