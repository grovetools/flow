...
func (g *GitClientAdapter) CreateBranch(name string) error {
	...
	if err := cmd.Run(); err != nil {
		...
		checkCmd := exec.CommandContext(ctx, "git", "-C", g.repoPath, "rev-parse", "--verify", name) //nolint:gosec // args are internal, not user input
		if checkCmd.Run() == nil {
...
func (g *GitClientAdapter) CurrentBranch() (string, error) {
	...
	cmd := exec.CommandContext(ctx, "git", "-C", g.repoPath, "rev-parse", "--abbrev-ref", "HEAD") //nolint:gosec // args are internal, not user input
...
func (g *GitClientAdapter) IsGitRepo() bool {
	...
	cmd := exec.CommandContext(ctx, "git", "-C", g.repoPath, "rev-parse", "--git-dir") //nolint:gosec // args are internal, not user input
	return cmd.Run() == nil
}
