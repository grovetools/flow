...
*   **`grove-notebook` (`nb`)**: Plan initialization hooks can execute `nb` commands to create plans from notes and users can view the notebook with the `grove nb tui` command. The notebook system acts as a separate store for development artifacts, independent of the main project repository. `flow` can also be configured to automatically preserve markdown files generated by Claude Code's "Plan Mode" into an executable job graph.
*   **`grove-context` (`cx`)**: Before executing `oneshot` or `chat` jobs, this tool is used to generate a (`grove cx generate`) context file based on `.grove/rules`. Context rules files can be saved and stored for different areas of focus.
...
*   **`grove-tmux` (`gmux`)**: Worktrees created by `flow` can be navigated using `gmux`, which provides Harpoon-inspired, single-key navigation. The `gmux key manage` TUI allows binding repositories and worktrees to hotkeys, `gmux history` provides quick access to recent workspaces, and `gmux windows` manages multiple agent windows within a session.
