...
Grove Flow is configured through YAML files at both the project and plan level. This layered approach allows for setting defaults and managing workflows.
...
A `.grove-plan.yml` file inside a plan's directory overrides project-level settings for that specific plan, making plans self-contained.
...
Grove Flow resolves settings in the following order of precedence (highest to lowest):
...
-   `GROVE_CONFIG`: Specifies a path to a custom `grove.yml` file, overriding the default search behavior.
...
-   Set frequently used models and container images in the project `grove.yml` to reduce repetition.
